...
def clogger(dir_path,userprog,lang,testcase):    
    VALGRIND_MSG_RE = re.compile('==\d+== (.*)$')
    end_of_trace_error_msg = None
    
    DN = dir_path#os.path.dirname(sys.argv[0])
    if not DN:
    ...
    USER_PROGRAM = userprog
    LANG = lang #'c'#sys.argv[2] # 'c' for C or 'cpp' for C++
    TESTCASE = testcase   
    ...
    VGTRACE_PATH = os.path.join(DN, temp+'.vgtrace')
    EXE_PATH = os.path.join(DN, temp+'.exe')
    ...
    if gcc_retcode == 0:
        ...
        (valgrind_stdout, valgrind_stderr) = valgrind_p.communicate(input=TESTCASE)
	...
	#print >> sys.stderr, '=== postprocess stderr ==='
        ...
        out.write(postprocess_stdout.decode("ISO-8859-1"))
        try:
            os.remove(F_PATH)
        except OSError:
            pass
        try:
            os.remove(VGTRACE_PATH)
        except OSError:
            pass
        try:
            os.remove(EXE_PATH)
